...
from .gas_cycle.forward import step_concentration_gems
...
class FAIR:
    ...
    co2_method : str
        method to use for calculating atmos co2. Valid options are
        {"default", "swann2010"}.
    ...
    ------
    ValueError
        if ``ghg_method`` or ``ch4_method`` or ``co2_method`` given are not valid options.
    """
    ...
    @property
    def co2_method(self):
        """Return co2 emissions to concentration method."""
        return self._co2_method.lower()

    @co2_method.setter
    def co2_method(self, value):
        if value.lower() in ["default", "swann2010"]:
            self._co2_method = value.lower()
        else:
            raise ValueError(
                f"co2_method should be ``default`` or ``swann2010``; you "
                f"provided {value.lower()}."
            )
    # gems
    @property
    ...
    def allocate(self):
        ...
        self.gas_partitions = xr.DataArray(
         ...
         # greta mod:
        self.co2_partitions = xr.DataArray(
            np.zeros(
                (self._n_timebounds,
                 self._n_scenarios,
                 self._n_configs,
...
# #             gems add
            for gasbox in range(self._n_gasboxes):
                if filename != DEFAULT_SPECIES_CONFIG_FILE:
                    try:
                        ...
                        fill(
                            self.species_configs["partition_fraction"],
                            df.loc[specie, f"partition_fraction{3}"],
                            specie=specie,
                            gasbox=gasbox,
                        )
                    try:
                        fill(
                            ...
                            df.loc[specie, f"unperturbed_lifetime{3}"],
                            specie=specie,
                            gasbox=gasbox,
                        )
                    except:
    ...
    def run(self, progress=True, suppress_warnings=True, 
        ...
        co2_partitions_array = self.co2_partitions.data
        flux_array = self.flux.data
...
#                     first, use default step_concentration to compute other ghgs
                    (
                        concentration_array[
                            ...
                            ...,
                            self._co2_indices==False,
                        ],
                        gas_partitions_array[..., self._co2_indices==False, :],
                        airborne_emissions_array[
                            i_timepoint + 1 : i_timepoint + 2,
                            ...,
                            self._co2_indices==False,
                        ...
                        baseline_concentration_array[
                            None, None, ..., self._co2_indices==False
                        ],
                        baseline_emissions_array[
                            None, None, ..., self._co2_indices==False, None
                        ],
...
#                     second, use default step_concentration_gsm to compute other co2 carbon cycle
                    for j_config in range(self._n_configs):
                        (
                            ...
                            gasboxes_old = gas_partitions_array[
                                ...,
                                j_config : j_config + 1,
                                self._co2_indices, :
                            ],  # last timebound
                            airborne_emissions_old = airborne_emissions_array[
                                i_timepoint + 1 : i_timepoint + 2,
                                ...,
                                j_config : j_config + 1,
                                self._co2_indices,
                            ...
                            baseline_concentration = baseline_concentration_array[
                                None, None, ..., j_config : j_config + 1, self._co2_indices
                            ],
                            baseline_emissions  = baseline_emissions_array[
                                None, None, ..., j_config : j_config + 1, self._co2_indices, None
                            ],
                            concentration_per_emission = concentration_per_emission_array[
                                None,
                                None, 
                                ..., 
#                                 j_config : j_config + 1,
#                                 ...,
                            ...
                            lifetime = unperturbed_lifetime_array[
                                None, None, 
#                                 ..., 
                                j_config : j_config + 1, self._co2_indices, :
                            ],
                            #        oxidation_matrix,
                ...
                # # gems
                co2_partitions_array[i_timepoint+1,...] = np.squeeze(gas_partitions_array) #gems added
                # # gems
